...
# END Problem EC
    ...
    def action(self, gamestate): #overrides ThrowerAnt.action 
        ...
        place = self.place.exit # place behind the queen ant  
        while place is not None:
            if place.ant is not None:
                if not place.ant.is_doubled:
                    damage = place.ant.damage * 2 
                    place.ant.damage = damage
                    place.ant.is_doubled = True 
            place = place.exit


    def reduce_armor(self, amount): #overrides Insect.reduce_armor 
...
class GameState:
    ...
    @property
    def bees(self):
        return [b for p in self.places.values() for b in p.bees] #list of lists that contains bee objects of all places 
