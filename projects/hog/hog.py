...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        while opponent_score != 0:
            s_fun = digit_fn(opponent_score % 10)
            score = s_fun(score)
...
def hog_pile(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    # END PROBLEM 4
    ones_digit = lambda x : x % 10 
    if ones_digit(player_score) == ones_digit(opponent_score):
        return ones_digit(opponent_score)
    return 0
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        who = next_player(who) # next player 
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    cmp = lambda s0, s1: s0 > s1 
    check_leader = lambda l, ll : l == ll

    leader = None 
    if cmp(score0, score1):
        leader = 0 
        if check_leader(leader, last_leader):
            return last_leader, None 
        return leader, f"Player {leader} takes the lead by {score0 - score1}" 
    elif cmp(score1, score0):
        leader = 1
        if check_leader(leader, last_leader):
            return last_leader, None 
        return leader, f"Player {leader} takes the lead by {score1 - score0}" 
    else:
        return None, None 
    # END PROBLEM 6
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    for i in range(1, 11):
        temp_avg = averaged(i, dice)
        if temp_avg > highest_average:
            max_scoring_num = i 
            highest_average = temp_avg
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    strat = lambda f : f(score, opponent_score)  
    is_eq = lambda x, y : x == y 
    
    if is_eq(strat(hefty_hogs) + score, opponent_score):
        return 0
    elif strat(hefty_hogs) >= threshold:
        return 0
